...
// ───────── PyO3 seksjon (skjult for rust-analyzer uten feature) ─────────
#[cfg(feature = "python")]
mod pybridge {
    use super::*;
    use pyo3::prelude::*;
    ...
    pub fn rust_calibrate_session(
        ...
        Python::with_gil(|py| {
            ...
            if let Some(r) = result.reason { out.set_item("reason", r)?; } else { out.set_item("reason", py.None())?; }
    ...
    // Viktig: modulnavnet må være cyclegraph_core
    #[pymodule(name = "cyclegraph_core")]
    fn cyclegraph_core_py(_py: Python, m: &PyModule) -> PyResult<()> {
        ...
        m.add_function(wrap_pyfunction!(analyze_session_py, m)?)?;
