...
const Interactive3DCyclistProfile: React.FC<{ profile: ProfileData | null }> = ({ profile }) => {
  ...
  // All 9 profile settings with FULL three-tier info
  const profileSettings = [
    {
      ...
      editable: true,
      bullseye: {
        simple: "Your body weight is like the base load in physics. Heavier = more energy needed to fight gravity on climbs. Think of it as carrying a backpack uphill - every extra kg matters!",
        whyEditable: "This varies dramatically between riders (50-100kg+) and is THE biggest factor in climbing power. We need your exact weight for accurate modeling.",
    ...
    },
    {
      ...
      editable: true,
      bullseye: {
        simple: "Bike weight adds to total system mass. A lighter bike helps on climbs (less gravity to fight) but makes almost zero difference on flats. Think race bike (6.8kg) vs touring bike (12kg).",
        whyEditable: "Varies widely: carbon race bikes (6.8kg) to steel tourers (12kg+). Combined with your weight, determines total climbing resistance.",
        variance: "Moderate - 5kg variance typical (±2-3% power on climbs)",
      },
      techDeepDive: {
        formula: "Same gravitational formula as rider weight: Fg = (m_rider + m_bike) × g × sin(θ)",
        variables: [
    ...
    },
    {
      ...
      editable: true,
      bullseye: {
        simple: "Counter-intuitive: wider tires (28-32mm) actually roll FASTER than skinny tires (23-25mm) at same pressure! Modern science debunked the old 'skinny = fast' myth. Wider = better comfort + grip + speed.",
        whyEditable: "Tire width directly affects rolling resistance (Crr). 23mm vs 32mm can be 20% difference in Crr. We need your actual tire size.",
    ...
    },
    {
      ...
      editable: true,
      bullseye: {
        simple: "Tire rubber compound matters! Premium tires (GP5000, Corsa) have special rubber that bends/recovers faster = less energy lost. Budget tires can cost you 10-20W at speed. It's like running shoes - cheap ones make you work harder!",
        whyEditable: "Tire quality affects Crr dramatically. Premium (Crr ~0.0030) vs Budget (Crr ~0.0055) = 45% difference! We use your tire quality to adjust Crr calculation.",
    ...
    },
    {
      ...
      editable: false,
      bullseye: {
        simple: "700c is the standard road bike wheel size (622mm rim diameter). We default to this because 95% of road bikes use it. Different wheel sizes change how Crr and speed are calculated.",
        whyLocked: "99% of road bikes are 700c. The few exceptions (650b, 26\", 29\") are obvious (gravel/MTB). Asking everyone is unnecessary complexity. Our 700c default is accurate for the vast majority.",
    ...
    },
    {
      ...
      editable: false,
      bullseye: {
        simple: "CdA is your 'air parachute' - how much wind you catch. Lower = slice through air easier. Road position (~0.300) vs TT position (~0.200) = 33% less wind resistance! This is THE biggest factor at high speeds.",
        whyLocked: "CdA is VERY hard to measure accurately (requires wind tunnel or fancy sensors). Most riders in road position are 0.280-0.320. Our 0.300 default is the sweet spot for typical road riding. Customizing would require expensive testing.",
    ...
    },
    {
      ...
      editable: false,
      bullseye: {
        simple: "Crr measures how much energy is lost as your tire deforms and un-deforms as it rolls. Lower Crr = less energy wasted. We calculate your Crr based on the tire width and quality you specified - no need to enter it manually!",
        whyLocked: "Crr is derived from your tire inputs (width + quality). Since you already told us your tire specs, we calculate the appropriate Crr scientifically. Manual entry would be redundant and error-prone.",
      ...
      techDeepDive: {
        ...
        calculation: "We use your tire_width + tire_quality to lookup scientific Crr from research database",
      },
    },
    {
      id: 'bike-type',
      ...
      editable: false,
      bullseye: {
        simple: "Bike type sets baseline assumptions for geometry and riding position. Road bikes = drop bars, aggressive position. Gravel = more upright, wider tires. TT bikes = super aero position. We use this to set smart defaults.",
        whyLocked: "Bike type is a 'meta-parameter' that helps set other defaults (typical CdA, typical Crr, typical geometry). Since you specified your actual tire specs, and we assume road-standard geometry, asking bike type adds little precision for most users.",
    ...
    },
    {
      ...
      editable: false,
      bullseye: {
        simple: "Not all your pedaling power reaches the wheel - some is lost to friction in the chain, cassette, and pulleys. Clean, well-lubed drivetrains are ~96-98% efficient. Dirty, worn chains drop to ~93-95%. We assume proper maintenance.",
        whyLocked: "Drivetrain efficiency varies little with good maintenance (96-98%). Measuring it requires lab equipment. Our 96% default assumes you maintain your bike properly (which we assume for road cyclists). The accuracy gain from customization is <1W.",
    ...
    },
    {
      ...
      editable: false,
      bullseye: {
        simple: "FTP (Functional Threshold Power) is the maximum power you can sustain for about an hour. It's your fitness baseline - we calculate this automatically from your ride data! No painful FTP test needed. We track how it changes over time.",
        whyLocked: "FTP is calculated, not input! We analyze your rides using physics-based power modeling to determine your FTP. This is one of our core features - automatic FTP tracking without field tests or power meters.",
      ...
      <div className="rounded-2xl bg-white/98 backdrop-blur-xl p-6 shadow-[0_20px_60px_rgba(0,0,0,0.25)] border border-slate-200">
        ...
        <div className="mb-6">
          ...
          <p className="text-sm text-slate-600 mb-3">
            ...
            <span className="font-semibold text-blue-600"> Hover over any setting to learn how it affects power calculations.</span>
          </p>
                    ...
                    <div className="flex items-center gap-2.5">
                      ...
                      <div className="flex-1">
                        ...
                        <div className="text-[11px] text-slate-500 mt-0.5 leading-tight">
                          {hoveredZone === setting.zone 
                            ? (setting.editable ? setting.variance : setting.variance)
                            : 'Hover to learn more'}
                        </div>
                      </div>
                    </div>
        ...
        {hoveredSetting && (
          <div className="mt-6 rounded-2xl border-2 bg-white shadow-lg overflow-hidden"
          ...
          >
              ...
              <div className="flex items-start gap-3 mb-4">
                ...
                <div className="flex-1">
                  ...
                  <p className="text-sm text-slate-700 leading-relaxed">
                    {hoveredSetting.bullseye.simple}
                  </p>
                </div>
              </div>
              ...
              >
                <div className="flex items-start gap-2">
                  ...
                  <div>
                    ...
                    <p className="text-sm text-slate-700 leading-relaxed">
                      {hoveredSetting.editable ? hoveredSetting.bullseye.whyEditable : hoveredSetting.bullseye.whyLocked}
                    </p>
                  </div>
                </div>
              </div>
            ...
            <div className="border-t-2" style={{ borderColor: hoveredSetting.color }}>
              ...
              >
                ...
                <svg 
                  className={`w-5 h-5 text-slate-600 transition-transform ${techExpanded ? 'rotate-180' : ''}`}
                  fill="none" 
                  viewBox="0 0 24 24" 
                  stroke="currentColor"
                >
              ...
              {techExpanded && (
                <div className="px-6 pb-6 space-y-4 bg-slate-50">
                  ...
                  {/* Variables */}
                  <div>
                    <div className="text-xs font-bold text-slate-700 uppercase tracking-wide mb-2">Variables:</div>
                    <div className="space-y-1">
                      {hoveredSetting.techDeepDive.variables.map((v, idx) => (
                        <div key={idx} className="text-sm text-slate-700 font-mono bg-white rounded px-3 py-1.5 border border-slate-200">
                          {v}
                        </div>
                      ))}
                    </div>
                  ...
                  {/* Impact */}
                  <div>
                    <div className="text-xs font-bold text-slate-700 uppercase tracking-wide mb-2">Real-World Impact:</div>
                    <div className="space-y-1">
                      {hoveredSetting.techDeepDive.impact.map((imp, idx) => (
                        <div key={idx} className="text-sm text-slate-700 bg-white rounded px-3 py-1.5 border border-slate-200">
                          {imp}
                        </div>
                      ))}
                    </div>
                  ...
                  {/* Edge Cases */}
                  <div>
                    <div className="text-xs font-bold text-slate-700 uppercase tracking-wide mb-2">Edge Cases:</div>
                    <div className="space-y-1">
                      {hoveredSetting.techDeepDive.edgeCases.map((edge, idx) => (
                        <div key={idx} className="text-sm text-slate-700 bg-amber-50 rounded px-3 py-1.5 border border-amber-200">
                          ⚠️ {edge}
                        </div>
                      ))}
                    </div>
                  ...
                  <div className="rounded-lg p-4"
                    style={{ backgroundColor: `${hoveredSetting.color}15`, borderLeft: `4px solid ${hoveredSetting.color}` }}
                  >
                    <div className="text-xs font-bold text-slate-700 uppercase tracking-wide mb-1">
                      Accuracy Impact:
                    </div>
                  ...
                  {/* Future Feature / Calculation (if exists) */}
